...
1. 先确认工具安装目录、任务根目录，以及在哪个系统运行 Codex。
...
- 委派前确认 MCP 可用且目录位于 HARNESS_MCP_ROOT 内。不得为了执行任务扩大
  允许目录、降低审批或关闭沙箱。缺失工具、凭证或权限时报告阻塞，不静默改为自己实现。
...
换项目时先检查当前 MCP 的固定 `--root`。项目在根目录内可直接指定子目录；
不在范围内则重新运行 setup，优先为新项目使用项目级配置。
用户级同名 `deepseek_harness` 配置更新会替换它的根目录，不会自动追加多个根目录；
不要在旧任务运行时切换配置。各项目的 `.env`、`.runtime` 与 Skill 分别维护。
...
确认后，在任务根目录内准备一个独立的测试子目录，再让 Codex：
...
同一系统、同一任务根目录的多个 MCP 进程共享调度状态，默认执行并发上限为 8，
超限任务进入队列；不同根目录拥有各自的并发上限。
...
| 环境变量 | 默认值 / 含义 |
| --- | --- |
| `HARNESS_MCP_ROOT` | 必填根目录；启动器的 `--root` 优先于此值 |
