...
async function setup(options, api) {
  ...
  try {
    ...
    const migration = info.legacy_mcp ? '（同时迁移旧 deepseek_harness 配置）' : '';
    console.log(`将合并 ${root}/.gitignore，${key ? '更新' : '保留'} .env，${info.existing_mcp ? '更新已有' : '新增'} dsh-in-codex MCP 配置${migration}。`);
